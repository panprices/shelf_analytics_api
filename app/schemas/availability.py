...
class HistoricalStockStatusItem(HistoricalItem):
    available_count: int = Field(
        description="Number of 'in_stock' (or similar) products", example=514
    )
    unavailable_count: int = Field(
        description="Number of 'out_of_stock' (or similar) products", example=32
    )


class HistoricalStockStatus(BaseModel):
    history: List[HistoricalStockStatusItem] = Field(
        description="The historical values"
    )
